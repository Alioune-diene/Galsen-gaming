...
let generateCartItems = () => {
  if (basket.length !== 0) {
        ...
        <div class="details">
          ...
          <div class="title-price-x">
            <h4 class="title-price">
              <p>${name}</p>
              <p class="cart-item-price">${price} FCFA</p>
            </h4>
    ...
    <h2>Panier Vide.</h2>
    <a href="index.html">
      <button class="HomeBtn">Acceuil</button>
    </a>
    `;
  }
};
...
let decrement = (id) => {
  ...
  update(selectedItem.id);
  //remoove all the unnecessary data, the datas wich have item of 0
...
let TotalAmount = () => {
  if (basket.length !== 0) {
    ...
    <h2>Total: ${amount} FCFA</h2>
    <button class="checkout">Valider</button>
    <button onclick="clearCart()" class="removeAll">Annuler</button>
    `);
  } else return;
};
